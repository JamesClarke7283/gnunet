           ...
           enum SuspendReason sr)
{
       ...
       slc = slc->next)
  {
    ...
    {
      GNUNET_log (GNUNET_ERROR_TYPE_DEBUG,
                  "Service name: %s and suspend state %u\n",
                  sh->service_name,
                  sh->suspend_state);
      if (0 == strcmp ("NOCASE", sh->service_name))
        continue;
    }
    else
      GNUNET_log (GNUNET_ERROR_TYPE_DEBUG,
                  "Service handle sh is NULL\n");
...
launch_registered_services (void *cls)
{
       ...
       shl = shl->next)
  {
    ...
    GNUNET_log (GNUNET_ERROR_TYPE_DEBUG,
                "Service name: %s and suspend state %u launching\n",
                sh->service_name,
                sh->suspend_state);
    shl->sh->cfg = cfg;
